...
//	}	
	...
	function getParentPosition( obj ) {
		console.log( '---', obj );
		_this.objPos.add( obj.position );
		
		if ( obj.parent != undefined ) {
			
			getParentPosition( obj.parent );
			
		}
		
	}

	getParentPosition( this.object );
	...
	this.prevPosition = new THREE.Vector3().copy( /*this.object.position*/ this.objPos );
	...
	function mouseup ( event ) {
		event.stopPropagation();
		_this.prevPosition.copy(/*_this.object.position*/ this.objPos );
		clicked = false;
	}
